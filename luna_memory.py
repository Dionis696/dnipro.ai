...
def save_fact(key, value):
    temp_facts[key] = (value, time.time())
    return f"Ок, запам'ятала: {key} — {value} 😏"

def get_fact(key):
    if key in temp_facts:
        value, timestamp = temp_facts[key]
        # Перевірка: чи минуло більше 12 годин (43200 секунд)
        if time.time() - timestamp < 43200:
            return value
        else:
            del temp_facts[key] # Стираємо, бо час вийшов
    return None

def clear_old_facts():
    # Автоматичне чищення старих фактів
    for key in list(temp_facts.keys()):
        if time.time() - temp_facts[key][1] > 43200:
            del temp_facts[key]
